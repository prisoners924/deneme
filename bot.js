...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'amalser' || config.AFPLK == 'drips' || config.AFPLK == 'vava' || config.AFPLK == 'Zim') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'amalser' || config.AFPLK !== 'drips' || config.AFPLK !== 'vava' || config.AFPLK !== 'Zim') {
         console.log(
